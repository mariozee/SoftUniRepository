    ...
    public class Engine
    {
        ...
        protected virtual void CreateCharacter(string[] inputParams)
        {
            if (inputParams[5] == "Red")
            {
                ...
                {
                    case "healer":
                        Healer healer = new Healer(inputParams[2], int.Parse(inputParams[3]),
                            int.Parse(inputParams[4]), Team.Red);
                        this.characterList.Add(healer);
                        break;
                    case "mage":
                        Mage mage = new Mage(inputParams[2], int.Parse(inputParams[3]),
                            int.Parse(inputParams[4]), Team.Red);
                        this.characterList.Add(mage);
                        break;
                    case "warrior":
                        Warrior warrior = new Warrior(inputParams[2], int.Parse(inputParams[3]),
                            int.Parse(inputParams[4]), Team.Red);
        ...
        {
            Character ch = this.characterList.First(x => x.Id == inputParams[1]);
            switch (inputParams[2])
            {
                case "injection":
                    Injection injection = new Injection(inputParams[3]);
                    injection.Countdown = 3;
                    ch.AddToInventory(injection);
                    break;
                case "pill":
                    if (ch is IHeal)
                        break;
                    ...
                    pill.Countdown = 1;
                    ch.AddToInventory(pill);
                    break;
                case "shield":
                    ch.AddToInventory(new Shield(inputParams[3]));
                    break;
                case "axe":
                    if (ch is IHeal)
                        break;
                    ch.AddToInventory(new Axe(inputParams[3]));
                    break;
            }
        }
